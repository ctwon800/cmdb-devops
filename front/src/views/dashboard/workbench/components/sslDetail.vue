      ...
      <div class="monitor-box" :style="{flex: '1 1 calc(33.33% - 10px)', minWidth: '180px', maxWidth: '300px', margin: '10px'}">
        ...
        <el-col :span="20">
          <div class="enroll-time">
            <div class="enroll-text">监控证书数</div>
            <div class="enroll-number-domain_count"><h3>{{ data.ssl_total || 0 }}</h3></div>
          </div>
        </el-col>
      </div>
      <div class="monitor-box" :style="{flex: '1 1 calc(33.33% - 10px)', minWidth: '180px', maxWidth: '300px', margin: '10px'}">
        ...
        <el-col :span="20">
          <div class="enroll-time">
            <div class="enroll-text">即将到期证书</div>
            <div class="enroll-number"><h3>{{ data.ssl_expiring_soon_name || 0 }}</h3></div>
            <div class="enroll-text">即将到期天数</div>
            <div class="enroll-number-day">
              <h3 :style="{color: data.ssl_expiring_soon_days < 31 ? 'red' : ''}">{{ data.ssl_expiring_soon_days || 0 }}</h3>
            </div>
          </div>
        </el-col>
      </div>
...
export default {
  ...
  methods: {
    initGet () {
      request({
        url: '/api/system/datav/ssl_detail/'
      }).then((res) => {
        this.data = res.data
      })
    },
